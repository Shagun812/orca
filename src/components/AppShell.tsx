  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-transparent">
      ...
      <header className={`absolute top-0 left-0 w-full z-50 flex items-center justify-between px-8 transition-all duration-500 ease-out ${
        scrolled
          ? 'h-16 bg-black/70 backdrop-blur-3xl border-b border-white/[0.08] shadow-[0_4px_30px_rgba(0,0,0,0.5)]'
      ...
      {/* Main content area */}
      <main className="flex-1 overflow-auto relative pt-24" onScroll={handleScroll}>
        <Outlet />
      </main>
    </div>
  )

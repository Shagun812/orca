...
pub async fn detect_spill(
    ...
    .bind(body.title.clone().unwrap_or_else(|| format!("Detection: {}", &body.image_id)))
    .bind(body.description.clone().unwrap_or_else(|| format!("Automated investigation for satellite image {}", &body.image_id)))
    .fetch_one(&pool)
    .await
